...
MonteCarloIntegration::MonteCarloIntegration(MathFunction<double>& f) : m_f(f), m_numSamples(DEFAULT_NUM_SAMPLES) 
{
}

MonteCarloIntegration::MonteCarloIntegration(const MonteCarloIntegration& p) : m_f(p.m_f), m_numSamples(p.m_numSamples)
{
}
...
int main ()
{
    ...
    double integral = mci.getIntegral(0.5, 2.5);
    cout << " the integral of the function is " << integral << endl;

    mci.setNumSamples(20000);
    integral = mci.getIntegral(0.5, 2.5);
